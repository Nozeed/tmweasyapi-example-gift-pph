...
if ($_POST["data"]) {
	...
	if (($ts - $data_api["timestamp"]) < 30) {
		$signature = md5($_POST["data"] . ":" . $tmweasy["api_key"]);
		if ($signature == $_POST["signature"]) {
			//---------------------------------อัพเดทฐานข้อมูลเครดิตรลูกค้่า เมื่อเติมสำเร็จ---------------------------------------------------------------------------------------------------
			$point = $data_api["amount"];
			$ref1 = $data_api["ref1"];
			$stmt = $conn->prepare("update {$database_set["table"]} set {$database_set["point_field"]} = {$database_set["point_field"]} + :point where {$database_set["user_field"]} = :ref1");
